...
from app.models import Course, Enrollment, User, ClassSlot, Lesson, db
...
def my_courses():
    ...
    for e in enrollments:
        ...
        if e.paid and e.paid_until and e.paid_until < date.today():
            ...
            db.session.commit()
        slots = ClassSlot.query.filter_by(
            course_id=course.id
        ).all()
...
def profile_data():
    ...
    "child_class_level":current_user.child_class_level,
...
def update_profile():
    ...
    current_user.child_name=data.get("child_name")
    current_user.child_class_level=data.get("child_class_level")
...
def course_content(course_id):
    ...
    if not enrollment or not enrollment.paid:
        return jsonify({
            "error":"You need to pay for this course first"
        }),403
...
@student_bp.route("/course-page/<int:course_id>")
@login_required
def course_content_page(course_id):
