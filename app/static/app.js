document.addEventListener("DOMContentLoaded", () => {
    ...
    window.deleteCourse = async function(id) {
        ...
        if (!confirm("Delete this course? This can't be undone.")) {
            return;
        }
    ...
    async function loadSlots(courseId) {
        ...
        let slotsHtml = slots.map(s => `
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; padding:8px 0; border-top:1px solid var(--line); font-size:0.9rem;">
                ...
                onclick="deleteSlot(${s.id}, ${courseId})">
        ...
        if (slots.length === 0) {
            ...
            slotsHtml =
            `<p style="font-size:0.9rem; color:var(--slate);">
        ...
        panel.innerHTML = `
            ...
            <div class="card" style="background:var(--paper);">
                ...
                <form onsubmit="addSlot(event, ${courseId})"
                style="margin-top:14px; padding-top:0;">
                    ...
                    <input type="text"
                    id="time-${courseId}"
                    placeholder="Time (e.g. 4:00 PM WAT)"
                    required>
    ...
    window.addSlot = async function(event, courseId) {
        ...
        const data = {
            ...
            meeting_link:
            document.getElementById(`link-${courseId}`).value
    ...
    payments.forEach(p => {
        ...
        <tr>
            ...
            <td>
                <span style="
                color:${p.paid ? 'var(--accent-deep)' : 'var(--danger)'};
                font-weight:600;">
                    ${p.paid ? '✓ Paid' : '✗ Pending'}
                </span>
            </td>
            ...
            ${
                ...
                <button 
                class="btn btn-accent"
                onclick="generateCode(${p.enrollment_id})">
                ...
                <button 
                class="btn edit"
                onclick="markPaid(${p.enrollment_id})">
    ...
    window.generateCode = async function(enrollmentId) {
        ...
        const res = await fetch(
            `/admin/enrollment/${enrollmentId}/generate-code`,
            {
        ...
        const codeBox =
        document.getElementById(`code-${enrollmentId}`);
    ...
    window.markPaid = async function(enrollmentId) {
        ...
        await fetch(`/admin/mark-paid/${enrollmentId}`, {
